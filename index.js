...
class m extends l {
    ...
    tick(t) {
        h() && !n()
            ? ((this.vx = -t(this.speed)), (this.faceLeft = !0))
            : n() && !h()
                ? ((this.vx = t(this.speed)), (this.faceLeft = !1))
                : (this.vx = 0),
            (this.walking = h() || n());
    }
    get faceLeft() {
    ...
    constructor(t) {
        super(document.getElementById("title")),
            ...
            (this.items = [].slice.call(
                this.element.querySelectorAll(".menu .item")
            )),
            (this.selected = w);
    }
    keydown({ key: t }) {
...
(D.prototype.createFxNodes = function () {
    ...
    (D.prototype.slide = function (t, e, s) {
        ...
        return (
            this.setFrequency(this.notes[t].frequency, e + h),
...
(R.staccato = 0.3),
    (z.staccato = 0.5),
    (R.waveType = "sine"),
    (R.gain.gain.value = 0.7),
    (z.gain.gain.value = 0.3),
    (L.staccato = 0.5),
    (C.staccato = 0.3),
    (M.staccato = 0.5),
    (_.staccato = 0.5),
    ...
    (C.waveType = "sine"),
    (M.waveType = "sine"),
    (L.gain.gain.value = 0.4),
    (C.gain.gain.value = 0.6),
    (M.gain.gain.value = 0.4),
    (_.gain.gain.value = 0.4);
let N = 1,
    U = "up";
setInterval(function () {
    "up" === U && (N += 1) > 9 && ((U = "down"), (N -= 1)),
        "down" === U && (N -= 1) < 1 && ((U = "up"), (N += 2)),
        (F.gain.gain.value = 0.01 * N),
        (S.gain.gain.value = 0.01 * N);
}, 300);
const I = () => {
    L.play(),
        M.play(),
...
(O.loop = !1),
    ...
    ($.waveType = "sawtooth"),
    ($.bass.gain.value = 10),
    (O.gain.gain.value = 0.3),
    (W.gain.gain.value = 0.6),
    ($.gain.gain.value = 0.4),
    (J.gain.gain.value = 0.3),
    (P.gain.gain.value = 0.3);
const H = 768,
    V = 480;
class K extends l {
    constructor(t) {
...
class j extends l {
    ...
    set width(t) {
        (super.width = t),
            ...
            this.element.setAttribute("width", 16 * Math.round(this.width / 16));
    }
    get height() {
        return super.height;
    }
    set height(t) {
        (super.height = t),
            ...
            this.element.setAttribute("height", 16 * Math.round(this.height / 16));
    }
    get on() {
        return !!this._on;
    }
    ...
    toJSON() {
        return [
            Math.round(this.x),
            Math.round(this.y),
            this.isUp || this.isDown ? 16 * Math.round(this.width / 16) : this.width,
            this.isLeft || this.isRight
                ? 16 * Math.round(this.height / 16)
                : this.height,
            Number(this.on),
            this.direction,
        ];
...
class Y extends l {
    constructor(s) {
        super(document.getElementById("controls")),
            ...
            (this.keys = [
                ...
                new X("key-a", () => t.has("a")),
                new X("key-d", () => t.has("d")),
                new X("key-space", () => t.has(" ")),
                new X("button-toggle", () => e.has(1)),
                new X("button-jump", () => e.has(0)),
                new X("button-left", () => e.has(14)),
                new X("button-right", () => e.has(15)),
            ]);
    }
    keydown({ key: t }) {
        switch (t) {
    ...
    tick() {
        if (!this.hidden) for (const t of this.keys) t.tick();
    }
}
const Q = 3,
    Z = document.getElementById("editor"),
    tt = Z.createSVGPoint(),
    et = ({ clientX: t, clientY: e }) => (
...
document.addEventListener("mouseup", () => {
    ...
    document.addEventListener("mousemove", (t) => {
        if (!st) return;
        const { x: e, y: s } = et(t);
        st({ x: e - it.x, y: s - it.y }), (it = { x: e, y: s });
    }),
    ...
    resize(t, { x: e, y: s }) {
        switch (t) {
            ...
            case "w":
                ...
                break;
            case "nw":
                this.resize("n", { x: e, y: s }), this.resize("w", { x: e, y: s });
                break;
            case "ne":
                this.resize("n", { x: e, y: s }), this.resize("e", { x: e, y: s });
                break;
            case "sw":
                this.resize("s", { x: e, y: s }), this.resize("w", { x: e, y: s });
                break;
            case "se":
                this.resize("s", { x: e, y: s }), this.resize("e", { x: e, y: s });
        }
    }
    region(t, e) {
        return (
...
class at extends l {
    ...
    set level(t) {
        ...
        for (this.guy.load(...e), this.goal.load(...s); this.bars.length;)
        ...
        if (!this.hidden)
            switch (e) {
                ...
                case "r":
                    ...
                    this.addSpike(
                        "u" === e || "d" === e
                            ? new nt(0, 0, 64, 8, !0, "u" === e ? "up" : "down")
                            : new nt(0, 0, 8, 64, !0, "l" === e ? "left" : "right")
                    );
                    break;
                case "h":
                    document.getElementById("dialog").hidden = !document.getElementById(
                        "dialog"
                    ).hidden;
                    break;
                case "g":
...
class ct extends l {
    ...
    set index(t) {
        for (
            this._index = Math.min(this.levels.length, Math.max(t || 0)),
            this.on = !0,
        ...
        for (; this.spikes.length;) this.spikes.pop().remove();
        if (this.finished)
            return (this.guy.hidden = !0), (this.congrats.hidden = !1), void T();
    ...
    lost() {
        return (
            this.guy.bottom > V ||
            this.bars.some((t) => t.on === this.on && t.overlaps(this.guy)) ||
            this.spikes.some((t) => t.on === this.on && t.overlaps(this.guy))
        );
    }
    setBounds(t) {
        const { bounds: e } = t;
        ...
        for (const s of this.bars)
            s.on === this.on &&
                (s.top < t.bottom &&
                    s.bottom > t.top &&
                    (s.isRightOf(t)
                        ? (e.right = Math.min(e.right, s.left - t.right))
                        : s.isLeftOf(t) && (e.left = Math.max(e.left, s.right - t.left))),
                    s.left < t.right &&
                    s.right > t.left &&
                    (s.isBelow(t)
                        ? (e.bottom = Math.min(e.bottom, s.top - t.bottom))
                        : s.isAbove(t) && (e.top = Math.max(e.top, s.bottom - t.top))));
        return e;
    }
    tick(t) {
        ...
        (this.guy.y += Math.min(n, Math.max(h, this.guy.vy))),
            0 === n
                ? ((this.guy.vy = i() ? -t(1200) : 0), i() && O.play())
                : (this.guy.vy = Math.min(t(600), this.guy.vy + t(120))),
            this.lost()
                ? this.death()
                : this.guy.overlaps(this.goal) && this.advance();
    }
}
const dt = new (class {
    constructor() {
        ...
        " " === t.key && this.toggle(),
            this.scene.hidden
                ? this.controls.hidden
                    ? this.title.hidden || this.title.keydown(t)
                    : this.controls.keydown(t)
                : this.scene.keydown(t);
    }
    get state() {
        return this._state;
    }
